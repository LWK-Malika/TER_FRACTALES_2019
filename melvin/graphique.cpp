...
double pixelToRepereX(int pixel){
  ...
  double x=(tailleX/800)*pixel+xmin;
  // cout<<" pixtorep= "<<x<<endl;
  return x;
}
...
double distPixToRepX(int pixel){

  return pixelToRepereX(pixel)-pixelToRepereX(0); 
}

double distPixToRepY(int pixel){
...
void remplirTab(){
  ...
  for (int i=0 ; i<800 ; i++) {
    ...
    for (int j=0 ; j<800 ; j++) {
      tab[i][j]=point::diverge((cadre.getTailleX()/800)*i+
			       cadre.getXmin(),
		 (cadre.getTailleY()/800)*j+
			       cadre.getYmin());
    }
  }
}
...
*/
  ...
  glBegin(GL_LINES); //a quoi ça sert ?
  glVertex2f(cadre.getTailleX()/800*-1+cadre.getXmin(),ymin);
  glVertex2f(cadre.getTailleX()/800*-1+cadre.getXmin(),ymax);
  glEnd();      	// Fermer le polygone
  glFlush();
...
void touche(int key, int x, int y){
  ...
  double distMoveX=distPixToRepX(20);
