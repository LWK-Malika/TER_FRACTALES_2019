...
// int nb_colonne = 800;
// int nb_ligne = 800;
// int valeur_initial = 0;
// vector <vector <int> > matrice;
// matrice.resize(nb_ligne, vector <int> (nb_colonne, valeur_initial));
// --- fin vector ---
...
int julia(double r, double i){
  double ZnR=r;
    ...
    while(sqrt(pow(ZnR,2)+pow(ZnI,2))<2 && cmp<100){
      ...
      ZnR=pow(ZnR,2)-pow(ZnI,2)+cr;    
      ZnI=2*temporaire*ZnI+ci;
      cmp++;
  }
  if(cmp==100)
    return -1;
...
void Rafraichir(void) {
  ...
  */ 
  ...
  for (int i=0 ; i<800 ; i++) {
    for (int j=0 ; j<800 ; j++) {
	    tab[i][j] = julia((tailleX / 800) * i + xmin, (tailleY / 800) * j + ymin);	
    }
  }
